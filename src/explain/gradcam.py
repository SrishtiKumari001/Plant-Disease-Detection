...
def _normalize_heatmap(heatmap: np.ndarray) -> np.ndarray:
    ...
    denom = float(heatmap.max()) + 1e-8
    if denom > 0:
        heatmap /= denom
    return np.clip(heatmap, 0.0, 1.0)


def _sparsify_heatmap(heatmap: np.ndarray, percentile: float = 75.0) -> np.ndarray:
    """
    Keep only the strongest activations so the overlay does not flood the image.
    Values below the percentile threshold are faded out.
    """
    ...
    sparse = np.where(heatmap >= cutoff, heatmap, 0.0)
    max_val = float(sparse.max())
    if max_val > 0.0:
        sparse /= max_val
...
def heatmap_to_rgba(heatmap: np.ndarray) -> np.ndarray:
    """
    Convert (H,W) heatmap 0..1 to RGBA using a perceptually smoother colormap.
    Alpha is tied to the activation strength so low-importance regions stay transparent.
    """
    ...
    rgba[..., 3] = heatmap  # transparency follows saliency
...
def overlay_heatmap_on_image(
    ...
    heatmap: np.ndarray,
    alpha: float = 0.45,
    activation_percentile: float = 75.0,
) -> Image.Image:
    """
    base_pil: PIL RGB, size (W,H)
    heatmap: (H,W) 0..1 at same spatial size as base_pil
    """
    heatmap = _sparsify_heatmap(heatmap, percentile=activation_percentile)
    ...
    # Scale saliency alpha by the requested blend strength.
    hm_arr = np.array(hm_rgba).astype(np.float32)
    hm_arr[..., 3] = np.clip(hm_arr[..., 3] * float(alpha), 0.0, 255.0)
    hm_rgba = Image.fromarray(hm_arr.astype(np.uint8), mode="RGBA")

    blended = Image.alpha_composite(base_rgba, hm_rgba).convert("RGB")
    return blended
