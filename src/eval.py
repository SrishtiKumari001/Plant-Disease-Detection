...
def load_label_names() -> Optional[List[str]]:
    """Load ordered class names from data/splits/label_map.json if present."""
    root = project_root()
    label_map_path = root / "data" / "splits" / "label_map.json"
...
def run_test(
    ...
    # Try to use label names from label_map.json so per-class outputs are human-readable
    class_names = load_label_names()
    if class_names is not None and len(class_names) != num_classes:
        # fallback to numeric names if mismatch
        class_names = None
