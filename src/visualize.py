...
# 1) Training curves
# -----------------------------
def plot_accuracy_curve(history_path: Path, out_dir: Path) -> Path:
    """
    Plots validation accuracy across epochs as a standalone figure.
    Also overlays val top-3 accuracy when it is available in history.json.
    """
    ...
    val_topk = [row.get("val_topk", None) for row in hist]
    phases = [row.get("phase", "") for row in hist]

    fig_path = out_dir / "accuracy_curve.png"

    plt.figure(figsize=(10, 5))
    plt.plot(steps, val_acc, marker="o", label="val_accuracy")

    if any(v is not None for v in val_topk):
        plt.plot(steps, val_topk, marker="o", label="val_top3_accuracy")

    if "finetune" in phases and "head" in phases:
        switch_idx = phases.index("finetune") + 1
...
def main() -> None:
    ...
    if history_path.exists():
        p0 = plot_accuracy_curve(history_path, out_dir)
        print(f"Saved: {p0}")
    ...
    # 4) misclassified gallery (optional)
